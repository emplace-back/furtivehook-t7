...
namespace events::instant_message
{
	...
	bool handle_message(const std::uint64_t sender_id, const char* message, const std::uint32_t message_size)
	{
		...
		instant_message::log_instant_messages(sender_id, message_size, type);
		return false;
	}
	...
	{
		const game::JoinSessionMessage message{ game::IMType::JOIN_REQUEST };
	...
	void add_friend_response(const std::uint32_t nonce, const game::InfoResponseLobby& lobby)
	{
		...
		const auto response = response_t{ nonce > 0, lobby.hostXuid, lobby.hostName, lobby.lobbyParams.mainMode, game::get_session_info(lobby), last_online };
	...
	void initialize()
	{
		...
		instant_message::on_message('h', [=](const auto& sender_id, auto& msg)
		{	
