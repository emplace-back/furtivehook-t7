...
namespace game
{
	...
	namespace oob
	{
		bool send(const game::netadr_t & target, const std::string & data, const netsrc_t & sock = NS_SERVER);
		...
		void send_lobby_msg(const game::netadr_t& to, const msg_t& msg, const LobbyModule module = LOBBY_MODULE_HOST);
	}
	...
	const static auto& spoof_t = reinterpret_cast<void*>(base_address + 0x2A95FFB);

	const static auto AngleDelta = reinterpret_cast<float(*)(const float, const float)>(game::base_address + 0x22A4800);
	const static auto AngleNormalize180 = reinterpret_cast<float(*)(const float)>(game::base_address + 0x701A0);
	...
	const static auto Material_RegisterHandle = reinterpret_cast<Material*(*)(const char *, int, bool, int)>(game::base_address + 0x1CD3B90);
	const static auto Com_SessionMode_IsMode = reinterpret_cast<bool(*)(eModes)>(game::base_address + 0x20F6DD0);
	const static auto MSG_LobbyState = reinterpret_cast<bool(__fastcall *)(void*, game::msg_t*)>(game::base_address + 0x1ED4320);
	const static auto MSG_JoinAgreementRequest = reinterpret_cast<bool(__fastcall *)(void*, game::msg_t*)>(game::base_address + 0x1EE0F90);
	const static auto MSG_ClientReliableData = reinterpret_cast<bool(__fastcall *)(void*, game::msg_t*)>(game::base_address + 0x1ED3EF0);
	const static auto MSG_ClientContent = reinterpret_cast<bool(__fastcall *)(Msg_ClientContent*, msg_t*)>(base_address + 0x1ED3DF0);
	const static auto MSG_JoinParty = reinterpret_cast<bool(__fastcall *)(void*, game::msg_t*)>(game::base_address + 0x1EE1110);
	...
	const static auto BG_seedRandWithGameTime = reinterpret_cast<unsigned int(*)(unsigned int*)>(game::base_address + 0x2687670);
	...
	const static auto BG_GetWeaponHitLocationMultiplier = reinterpret_cast<float(*)(const Weapon, uint16_t)>(game::base_address + 0x26F2B80);
	const static auto G_GetWeaponHitLocationMultiplier = reinterpret_cast<float(*)(uint16_t, Weapon)>(game::base_address + 0x19863C0);
	...
	const static auto LobbyMsgRW_PackageElement = reinterpret_cast<bool(__fastcall *)(msg_t *, bool)>(base_address + 0x1EF5610);
	...
	const static auto MSG_InitReadOnly = reinterpret_cast<void(*)(game::msg_t*, std::uint8_t*, int)>(base_address + 0x2154A70);
	...
	const static auto dwInstantSendMessage = reinterpret_cast<void(*)(ControllerIndex_t, const std::uint64_t*, unsigned int, char, const void *, unsigned int)>(base_address + 0x1439810);
	...
	const static auto Netchan_SendMessage = reinterpret_cast<bool(*)(ControllerIndex_t, NetChanMsgType, NetchanMsgMode, const char *, unsigned int, std::uint64_t, netadr_t, const char *)>(game::base_address + 0x2175A60);
	const static auto MSG_LobbyClientHeartbeat = *reinterpret_cast<bool(*)(Msg_LobbyClientHeartbeat*, msg_t*)>(game::base_address + 0x1ED4170);
	...
	const static auto& g_fHitLocDamageMult = reinterpret_cast<float*>(game::base_address + 0xA08C4D0);
	const static auto& g_assetNames = reinterpret_cast<const char**>(game::base_address + 0x3329080);
	...
	inline LobbySession* get_host_session(const LobbyType lobby_type)
	{
		if (lobby_type < LOBBY_TYPE_PRIVATE || lobby_type > LOBBY_TYPE_GAME)
		{
			return nullptr;
		}
	...
	inline LobbySession* get_client_session(const LobbyType lobby_type)
	{
		if (lobby_type < LOBBY_TYPE_PRIVATE || lobby_type > LOBBY_TYPE_GAME)
		{
			return nullptr;
